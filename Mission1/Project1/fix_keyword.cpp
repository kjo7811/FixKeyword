...
bool similer(const std::string& a, const std::string& b) {
	if (a.empty() && b.empty()) return 100; // 반환 타입 안맞음
	if (a.empty() || b.empty()) return 1;

	int dist = levenshtein(a, b); // 함수명 수정 (추상화)
...
// 너무 길고 중복이 많음
string FixKeyword(string w, string wk) {
	UZ++;

	int index = 0; // index 변수명 이상함
	...
	//재정렬 작업. 버그있음.
	if (UZ >= MAX_UZ || max1 >= MAX_UZ || max2 >= MAX_UZ) {
		UZ = 9;
		for (int i = 0; i < 5; i++) { //여기 버그
			int num = 1;
	...
	if (weekBest[index].size() == 10) { // ==이 맘에 안듦
